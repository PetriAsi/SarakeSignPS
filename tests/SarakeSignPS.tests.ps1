...
Describe "SarakeSignPS" {
    ...
    Context "Manifest, changelog, and AppVeyor" {
        ...
        # slightly to match SarakeSignPS.
        ...
        It "Includes a valid manifest file" {
            {
                $script:manifest = Test-ModuleManifest -Path "$moduleRoot\SarakeSignPS.psd1" -ErrorAction Stop -WarningAction SilentlyContinue
            } | Should -Not Throw
        }
        ...
        $script:manifest = Invoke-Expression (Get-Content "$moduleRoot\SarakeSignPS.psd1" -Raw)
        It "Manifest file includes the correct root module" {
            $script:manifest.RootModule | Should -Be 'SarakeSignPS'
        }
