...
task CreateHelp {
    ...
    New-ExternalHelp -Path "$BuildRoot\docs" -OutputPath "$BuildRoot\SarakeSignPS\en-US" -Force
    Remove-Module SarakeSignPS, platyPS
}
...
task UpdateManifest GetVersion, {
    Update-Metadata -Path "$releasePath\SarakeSignPS\SarakeSignPS.psd1" -PropertyName ModuleVersion -Value $script:Version
    #Updated by hand
...
$ConvertMarkdown = @{
    Inputs  = { Get-ChildItem "$releasePath\SarakeSignPS\*.md" -Recurse }
    Outputs = {process {
...
task Deploy -If (
    ...
    Remove-Module SarakeSignPS -ErrorAction SilentlyContinue
}, PublishToGallery

task PublishToGallery {
    assert ($env:PSGalleryAPIKey) "No key for the PSGallery"
    import-Module -Name Configuration
    Import-Module $releasePath\SarakeSignPS\SarakeSignPS.psd1 -ErrorAction Stop
    Publish-Module -Name SarakeSignPS -NuGetApiKey $env:PSGalleryAPIKey
}
...
task RemoveMarkdownFiles {
    Remove-Item "$releasePath\SarakeSignPS\*.md" -Force -ErrorAction 0
}
